...
#filename = raw_input('please print the filename you want to convert:) ')

filename = '37-node.xml'
...
def setHostRoutes(value):
    ...
    for i in range(len(alpha)):
       ipA = ('192.168.' + str(int(targets[alpha[i]]) + 1) + '.0/24')
       ipB = ('192.168.' + str(int(value) + 1) + '.' + str(int(targets[alpha[i]]) + 3))
       routes = {'destination' : ipA, 'nexthop' : ipB}
       ipAs.append(routes) 

    for i in range(len(belta)):
       ipA = ('192.168.' + str(int(sources[belta[i]]) + 1) + '.0/24')
       ipB = ('192.168.' + str(int(value) + 1) + '.' + str(int(sources[belta[i]]) + 3))
       routes = {'destination' : ipA, 'nexthop' : ipB}
       ipBs.append(routes) 
    return ipAs + ipBs
...
for i in range(len(Nes)):
    ...
    data[Nes[i]]['properties'] = {'network_id' : {'get_resource' : Nodes[i]},
				...
				'dns_nameservers' : ["8.8.8.8", "8.8.4.4"],
                                'cidr' : '192.168.' + str(i + 1) + '.0/24',
                                'host_routes' : setHostRoutes(i)
                                }
    ###########################################################

for i in range(len(internal_routers)):
    data[internal_routers[i]] = {'type' : 'OS::Neutron::Router'}
    data[internal_routers[i]]['properties'] = {'name' : internal_routers[i]}

for i in range(len(internal_rinterfaceAs)):
    data[internal_rinterfaceAs[i]] = {'type' : 'OS::Neutron::RouterInterface'}
    data[internal_rinterfaceAs[i]]['properties'] = {'router_id' : {'get_resource' : (sorted(internal_routers+internal_routers))[i]},
                                                    'port' : {'get_resource' : instancers_port0[i]}}

for i in range(len(internal_rinterfaceBs)):
    data[internal_rinterfaceBs[i]] = {'type' : 'OS::Neutron::RouterInterface'}
    data[internal_rinterfaceBs[i]]['properties'] = {'router_id' : {'get_resource' : (sorted(internal_routers+internal_routers))[i]},
                                                    'port' : {'get_resource' : instancers_port1[i]}}

for i in range(len(instancers_port0)):
    data[instancers_port0[i]] = {'type' : 'OS::Neutron::Port'}
    data[instancers_port0[i]]['properties'] = {'network' : {'get_resource' : Sources[i]},
                                            'fixed_ips': [{'ip_address' : '192.168.' + str(int(sources[i]) + 1) + '.' + str(int(targets[i]) + 3)}]}

for i in range(len(instancers_port1)):
    data[instancers_port1[i]] = {'type' : 'OS::Neutron::Port'}
    data[instancers_port1[i]]['properties'] = {'network' : {'get_resource' : Targets[i]},
                                            'fixed_ips': [{'ip_address' : '192.168.' + str(int(targets[i]) + 1) + '.' + str(int(sources[i]) + 3)}]}
